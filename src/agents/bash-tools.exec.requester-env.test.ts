/**
 * Regression test: exec tool exports OPENCLAW_REQUESTER_* env vars for subagent sessions.
 *
 ...
 * those values are exported as OPENCLAW_REQUESTER_* environment variables.
 */
...
describe("exec tool requester env vars", () => {
  ...
  it("exports OPENCLAW_REQUESTER_* env vars when requester context is provided", async () => {
    if (isWin) {
      return;
    } // Skip on Windows
    ...
    const result = await execTool.execute("test-call", {
      command: "env | grep OPENCLAW_REQUESTER || true",
    });

    const output = (result.details as { aggregated?: string })?.aggregated ?? "";

    expect(output).toContain("OPENCLAW_REQUESTER_CHANNEL=whatsapp");
    expect(output).toContain("OPENCLAW_REQUESTER_TO=group:120363123456789@g.us");
    expect(output).toContain("OPENCLAW_REQUESTER_THREAD_ID=msg-123");
    expect(output).toContain("OPENCLAW_REQUESTER_ACCOUNT_ID=37127301244");
  });

  it("exports OPENCLAW_SESSION_KEY alongside requester vars", async () => {
    if (isWin) {
      return;
    } // Skip on Windows
    ...
    expect(output).toContain(`OPENCLAW_SESSION_KEY=${sessionKey}`);
    expect(output).toContain("OPENCLAW_REQUESTER_CHANNEL=telegram");
    expect(output).toContain("OPENCLAW_REQUESTER_TO=dm:123456789");
  });
  ...
  it("does not export requester vars when not provided", async () => {
    ...
    const result = await execTool.execute("test-call", {
      command: "env | grep OPENCLAW_REQUESTER || echo 'none found'",
    });
